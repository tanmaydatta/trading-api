...
st['iq_buy_rsl'] = iq_buy_rsl
st['iq_sell_rsl'] = iq_sell_rsl
...
def home_st(request, strategy):
	if request.method == "POST":
		...
		if "rsl" in strategy:
			ws_obj.set_rsl(request.POST['R'], request.POST['S'], request.POST['L'])
		try:
	...
	if "rsl" in strategy:
		return render(request, "rsl.html")
	return render(request,"home.html")
...
def cancel(request, ins_token):
	kite = KiteConnect(api_key=trading_api.settings.API_KEY)
	kite.set_access_token(request.session['access_token'])
	orders = kite.orders()
	# #ipdb.set_trace()
	for order in reversed(orders):
		if str(order['instrument_token']) == str(ins_token) and ('OPEN' in order['status']) :
			kite.order_cancel(order['order_id'], order['variety'])
...
def flat(request, symbol):
	...
	st = symbol.split("$")[1]
	try:
		ticksize = threads['thread_' + str(ins_token) + '$' + str(st)].ticksize
	except:
		return response("fail", "tick size not found")
...
def check_connection_thread(h):
	global threads
	for key in threads:
		if threads[key].ws.is_connected() == False:
			try:
				threads[key].ws.connect(threaded=True)
			except:
				thread.start_new_thread( send_mail, ( threads[key], key+' stopped due to connection error. Could not connect even after retry','Symbol Stopped' , ) )
				threads.pop(key, None)

def check_connection(request):
	thread.start_new_thread(check_connection_thread, ("h", ))
